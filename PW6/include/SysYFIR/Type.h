...
private:
    ...
    std::weak_ptr<Type> result_;
    std::vector<std::weak_ptr<Type>>  args_;
};
...
private:
    ...
    std::weak_ptr<Type> contained_;   // The element type of the array.
...
private:
    ...
    std::weak_ptr<Type> contained_;   // The element type of the ptr.
};
